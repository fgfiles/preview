...
RocketGame.prototype.build = function () {
    ...
    window.addEventListener(
        "keydown", this.screenReleased.bind(this), false
    );
...
RocketGame.prototype.screenReleased = function (event) {
    ...
    if(event && event.keyCode && event.keyCode !== 32) {
        return
    }

    // If a native event and not a pixi event then prevent the default action.
