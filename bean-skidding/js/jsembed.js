...
var jsembed = function () {
    ...
    var widthIdeal = 750; // Default only, value set at runtime via jsembed.embed -- represents the "native" dimensions of the embeded JS app
    var heightIdeal = 500; // Default only, value set at runtime via jsembed.embed -- represents the "native" dimensions of the embeded JS app
    var scaleMax = 3; // The largest ratio we let the app scale to. 3 represents 300%.
    var widthMax = 750; // The maximum canvas width we'll allow. Only applies to FILL_TO
    var heightMax = 500; // The maximum canvas height we'll allow. Only applies to FILL_TO
	...
	*/
    ...
    function embedCustomAPIMain(tar, app) {
        ...
        case API_HAXE_FLAMBE:
            ...
            if (win.webfontoverlay == null)
                console.log("WARNING: webfontoverlay is null. Please Include the webfontoverlay.js before jsembed.");
            else
                win.webfontoverlay.initialize("webfontoverlaytarget", "embedtarget");
    ...
    function embedCustomAPIPost() {
        console.log("EMBED CUSOM API POST CALLED");
        switch (getCustomAPI()) {
        case API_CONSTRUCT_2:
